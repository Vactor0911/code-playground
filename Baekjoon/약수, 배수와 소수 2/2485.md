...
**문제 바로가기** : [2485 가로수](https://www.acmicpc.net/problem/2485 "2485 가로수")
...
aryGap = []
treePosOld = int(stdin.readline())
for _ in range(loops-1):
    treePos = int(stdin.readline())
    aryGap.append(treePos - treePosOld)
    treePosOld = treePos

gcd_ = aryGap[0]
for i in range(1, len(aryGap)):
    gcd_ = gcd(gcd_, aryGap[i])
