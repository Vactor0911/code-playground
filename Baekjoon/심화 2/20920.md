...
**문제 바로가기** : [20920 영단어 암기는 괴로워](https://www.acmicpc.net/problem/20920 "20920 영단어 암기는 괴로워")
...
words, length = map(int, stdin.readline().split())
...
aryWord = sorted(dictWord.items(), key = lambda x : (-x[1], -len(x[0]), x[0]))
for k in aryWord:
    print(k[0])
```
